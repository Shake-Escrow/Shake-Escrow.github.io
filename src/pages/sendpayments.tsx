...
const SendPayments: React.FC = () => {
  ...
  return (
    <div className="pt-24">
      <SEO
        title="Send Payments Safely with Escrow"
        description="Send crypto and web3 payments with built-in buyer protection and milestone escrow."
        canonical="/send-payments"
      />
      ...
      {[
        ...
        return (
          <div key={section.headline} className={`bg-[#e6e9ed] rounded-2xl py-8 px-6 md:px-12 md:py-12 flex flex-col items-start text-left h-full ${idx === 0 ? ' md:mb-0' : 'mt-8 md:mt-0'}`}>
            <img src={imageSrc} alt={section.headline} className="w-full h-43 object-cover rounded-2xl " />
            <h3 className="font-display font-bold text-2xl md:text-3xl text-secondary-dark mb-4" style={{letterSpacing: '-0.03em'}}>{section.headline.split('\n').map((line, idx) => (<React.Fragment key={idx}>{line}{idx < section.headline.split('\n').length - 1 && <br />}</React.Fragment>))}</h3>
            <p className="text-description mb-6 text-[#2d3440] opacity-90 font-body">{idx === 1 ? renderWithOdosLink(section.description) : section.description}</p>
          </div>
        );
      })}
    </div>
      ...
      <Section bgColor="bg-white" className="pb-0">
        <div className="max-w-5xl mx-auto rounded-3xl bg-primary-light p-8 md:p-12 flex flex-col md:flex-row gap-7 items-center">
          <div className="flex-1">
            <h2 className="font-display text-3xl text-secondary-dark">{siteContent.sendPayments.marketplace.headline}</h2>
            <p className="mt-4 font-body text-lg text-secondary">{siteContent.sendPayments.marketplace.description}</p>
          </div>
          <a href="https://t.me/Shake_Marketplace" target="_blank" rel="noopener noreferrer" className="shrink-0">
            <Button variant="outline" size="lg">{siteContent.sendPayments.marketplace.button}</Button>
          </a>
        </div>
      </Section>
